        ...
        <div className="max-w-5xl mx-auto grid lg:grid-cols-2 gap-6 sm:gap-8 md:gap-12">
          <div>
            ...
            <div className="flex flex-col gap-4">
              {[
                { icon: "📞", title: t('options.phone.title'), info: t('options.phone.info'), sub: t('options.phone.sub'), href: `tel:${t('options.phone.info').replace(/\s/g, '')}` },
                { icon: "💬", title: t('options.telegram.title'), info: t('options.telegram.info'), sub: t('options.telegram.sub'), href: "https://t.me/nondomgroup" },
              ].map((c) => (
          ...
          <div className="bg-white border-[1.5px] border-gray-200 rounded-2xl p-5 sm:p-6 md:p-8">
            ...
            <div className="flex flex-col gap-4">
              {[
                [t('form.name.label'), "text", t('form.name.placeholder')],
                [t('form.email.label'), "email", t('form.email.placeholder')],
                [t('form.company.label'), "text", t('form.company.placeholder')]
              ...
              <div>
                <label className="block text-sm font-bold text-[#0A1628] mb-1.5">{t('form.message.label')}</label>
                <textarea rows={4} placeholder={t('form.message.placeholder')} className="w-full border-[1.5px] border-gray-200 focus:border-[#0096C7] rounded-lg px-4 py-2.5 text-sm outline-none transition-colors duration-200 resize-none"/>
              </div>
              ...
              <p className="text-gray-400 text-xs text-center">{t('form.alternative')} <a href="https://non-dom.group/kontakt" target="_blank" rel="noopener noreferrer" className="text-[#0096C7] hover:underline">{t('form.alternativeLink')}</a></p>
            </div>
          </div>
        </div>
      </section>
